...
CREATE TABLE çalışanlar 
    ...
    INSERT INTO çalışanlar VALUES(123456789, 'Ali Seker', 'Istanbul', 2500, 'Vakko');
    INSERT INTO çalışanlar VALUES(234567890, 'Ayse Gul', 'Istanbul', 1500, 'LCWaikiki');
    INSERT INTO çalışanlar VALUES(345678901, 'Veli Yilmaz', 'Ankara', 3000, 'Vakko');
    INSERT INTO çalışanlar VALUES(456789012, 'Veli Yilmaz', 'Izmir', 1000, 'Pierre Cardin');
    INSERT INTO çalışanlar VALUES(567890123, 'Veli Yilmaz', 'Ankara', 7000, 'Adidas');
    INSERT INTO çalışanlar VALUES(456789012, 'Ayse Gul', 'Ankara', 1500, 'Pierre Cardin');
    INSERT INTO çalışanlar VALUES(123456710, 'Fatma Yasa', 'Bursa', 2500, 'Vakko');
...
select isim,maaş,işyeri from çalışanlar
where işyeri in (select marka_isim from markalar where çalışan_sayısı>15000);
    
/*----------------------------------------------------------------------------
  ORNEK2: marka_id’si 101’den büyük olan marka çalişanlarinin isim, maaş ve 
  şehirlerini listeleyiniz. 
  -----------------------------------------------------------------------------*/   
    select isim,maaş,şehir from çalışanlar
    where işyeri in(select marka_isim from markalar where marka_id>101);
    
/* -----------------------------------------------------------------------------
  ORNEK3: Ankara’da calisani olan markalarin marka id'lerini ve calisan
  sayilarini listeleyiniz.
  -----------------------------------------------------------------------------*/ 
  select marka_id,çalışan_sayısı from markalar
  where marka_isim in(select işyeri from çalışanlar where şehir='Ankara');
 ...
 -----------------------------------------------------------------------------*/
  select marka_isim,çalışan_sayısı,(select sum(maaş)from çalışanlar where marka_ismi=işyeri)from markalar;
