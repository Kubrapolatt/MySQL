...
-- group by ile where cümleciği kullanılabilir    
...
/* ----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/ 
	select isim,urun_adi,sum(urun_miktari) toplam
    from manav
 ...
 ----------------------------------------------------------------------------------------------*/     
    ...
    select isim,urun_adi,sum(urun_miktari) toplam_urun_mitari from manav
    group by isim,urun_adi
    having sum(urun_miktari)>=3
    order by sum(urun_miktari) desc;
    
   -- aggregate fonksiyonları ile ilgili koşul koymak için
   -- group by'dan sonra having kelimesi kullanılır
...
select sum(distinct urun_miktari)toplam_benzersiz from manav;
